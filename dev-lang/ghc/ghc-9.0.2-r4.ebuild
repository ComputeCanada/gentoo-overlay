...
relocate_ghc() {
	...
	if use prefix; then
		...
		patchelf --set-interpreter "${EPREFIX}/lib64/ld-linux-x86-64.so.2" "${WORKDIR}/usr/lib64/ghc-9.0.2/bin/ghc"
		patchelf --set-interpreter "${EPREFIX}/lib64/ld-linux-x86-64.so.2" "${WORKDIR}/usr/lib64/ghc-9.0.2/bin/ghc-pkg"
	fi
...
src_prepare() {
	...
	if use binary; then
		...
		eapply "${FILESDIR}"/${PN}-9.0.2-linker-script-basename.patch
