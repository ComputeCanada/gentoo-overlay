...
# A similar thing needs to be done for the cpuid library.
ARCHSPEC_JSON="${PN}-json-${PV}.tar.gz"
ARCHSPEC_CPUID_VERSION="5b381ed2af05ad9ead46a136b43c2f7021bd44bb"
ARCHSPEC_CPUID="${PN}-cpuid-${ARCHSPEC_CPUID_VERSION}.tar.gz"
SRC_URI+=" https://github.com/${PN}/${PN}-json/archive/v${PV}.tar.gz -> ${ARCHSPEC_JSON}"
SRC_URI+=" https://github.com/${PN}/cpuid/archive/${ARCHSPEC_CPUID_VERSION}.tar.gz -> ${ARCHSPEC_CPUID}"
...
src_unpack() {
	...
	tar -C "${P}"/"${PN}"/vendor/cpuid --strip-components=1 -xzf "${DISTDIR}/${ARCHSPEC_CPUID}" || die
}
