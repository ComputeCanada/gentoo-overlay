# Note: heavily modified to only build libtorque for build-time use for Open MPI
...
pkg_setup() {
	PBS_SERVER_HOME="${PBS_SERVER_HOME:-${EPREFIX}/var/spool/${PN}}"
...
src_prepare() {
	...
	eapply "${FILESDIR}"/${PN}-6.0.4-no-openssl.patch
	eapply_user
	mkdir -p "${S}"/m4
	eautoreconf
}
...
src_configure() {
	...
	econf \
		...
		--with-environ="${EPREFIX}"/etc/pbs_environment \
		--with-default-server=${PBS_SERVER_NAME} \
		--disable-gcc-warnings \
		--with-tcp-retry-limit=2 \
		--without-loadlibfile \
		--disable-mom --disable-clients --disable-gui
}
