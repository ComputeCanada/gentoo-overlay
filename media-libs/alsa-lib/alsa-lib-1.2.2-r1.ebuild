...
multilib_src_install_all() {
	...
	# null device is default to avoid warnings without sound cards
	sed -i "s@\(.*/etc/alsa/conf.d\"\)@\t\t\t\"$EPREFIX/etc/alsa/conf.d\"\n\1@" "${ED}"/usr/share/alsa/alsa.conf
	mkdir -p "${ED}"/etc/alsa/conf.d
	cat <<-EOF > "${ED}"/etc/alsa/conf.d/00-null.conf
	pcm.!default {
		type plug
		slave.pcm "null"
	}
