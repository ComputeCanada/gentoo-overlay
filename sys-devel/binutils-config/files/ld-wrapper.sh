...
if [ "$RSNT_DONT_SET_RPATH" != 1 ]; then
    ...
    if [ -n "$RSNT_LOCAL_MODULEPATHS" ]; then
	EASYBUILD_LOCAL_DIR=$(dirname "$RSNT_LOCAL_MODULEPATHS")
    fi
    ...
    addToRPath() {
        ...
        if [ "${1:0:${#EASYBUILD_DIR}}" != "$EASYBUILD_DIR" -a \
            ...
            "${1:0:${#EASYBUILD_HOME_DIR}}" != "$EASYBUILD_HOME_DIR" -a \
            "${1:0:${#EASYBUILD_LOCAL_DIR}}" != "$EASYBUILD_LOCAL_DIR" ]; then
            if [ -z "$origin_rpath" -a \
