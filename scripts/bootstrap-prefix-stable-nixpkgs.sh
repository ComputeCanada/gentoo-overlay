	...
	*) einfo "Don't know to unpack ${A}" ;;
	...
	case $CHOST in
		...
		*-linux*)
			...
			# the current arch
			libdir="-L$NIXUSER_PROFILE/lib/$(gcc ${CFLAGS} -print-multi-os-directory)"
		;;
...
bootstrap_stage2() {
	...
	EXTRA_ECONF=$(rapx --with-sysroot=$NIXUSER_PROFILE) \
	...
	for pkg in ${compiler_stage1} ; do
		...
		EXTRA_ECONF="--with-sysroot=/cvmfs/soft.computecanada.ca/nix/var/nix/profiles/glibc-2.24 --with-native-system-header-dir=/include --disable-bootstrap $(rapx --with-linker-hash-style=both) --with-local-prefix=${ROOT}" \
		MYCMAKEARGS="-DCMAKE_USE_SYSTEM_LIBRARY_LIBUV=OFF" \
		GCC_MAKE_TARGET=all \
		TPREFIX="${ROOT}" \
		PYTHON_COMPAT_OVERRIDE=python${PYTHONMAJMIN} \
		LIBRARY_PATH=$NIXUSER_PROFILE/lib \
...
bootstrap_stage3() {
	...
	emerge_pkgs() {
		...
		PREROOTPATH="${ROOT}"$(echo /{,tmp/}{usr/,}{,lib/llvm/{10,9,8,7,6,5}/}{s,}bin | sed "s, ,:${ROOT},g"):$NIXUSER_PROFILE/bin \
