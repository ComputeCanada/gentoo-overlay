...
src_prepare() {
	...
	sed -i \
		...
		-e "s:/usr/local/screens:/tmp:g" \
		-e "s:/local/etc/screenrc:${EPREFIX}/etc/screenrc:g" \
		-e "s:/etc/utmp:/var/run/utmp:g" \
		-e "s:/local/screens/S\\\-:/tmp/S\\\-:g" \
		doc/screen.1 || die
...
src_install() {
	...
	newtmpfiles - screen.conf <<<"d /tmp ${tmpfiles_perms} root ${tmpfiles_group}"
...
pkg_postinst() {
	...
	# !! DISABLE for installation on CVMFS that uses /tmp
